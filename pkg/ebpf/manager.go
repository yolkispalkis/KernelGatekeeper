...
// FIX: Use the type generated by bpf2go for connect4
type BpfNotificationTupleT = bpf_connect4NotificationTupleT // Renamed to match generated type

// --- Типы данных Go ---
// FIX: Define NotificationTuple struct
	...
	for _, closer := range closers {
		if c, ok := closer.(interface{ Close() error }); ok && c != nil {
			if closer != nil {
				...
				// FIX: Check for ringbuf.ErrClosed as well
...
func NewBPFManager(cfg *config.EBPFConfig, listenerIP net.IP, listenerPort uint16) (*BPFManager, error) {
	...
	opts := &ebpf.CollectionOptions{
		...
		Programs: ebpf.ProgramOptions{
			...
			// FIX: Use VerifierLogSize instead of LogSize
			VerifierLogSize: 1024 * 1024 * 4, // Increase verifier log size if needed
		},
	}
	...
	manager := &BPFManager{
		...
		statsCache: StatsCache{ // Initialize the cache
			// mu: sync.Mutex{}, // Zero value is fine
			lastTotalStats: GlobalStats{}, // Zero value is fine
			lastStatsTime:  time.Now(),    // Initialize time
		},
	}
	// statsCache initialization moved into struct literal
...
func (m *BPFManager) updateAndLogStats() error {
	...
	if err != nil {
		// Do not return error here, just log and skip update
		slog.Error("Failed to read BPF stats for updateAndLogStats", "error", err)
		// Update time to prevent rapid retries
		m.statsCache.lastStatsTime = now
		return nil // Don't propagate error, try again next interval
	}
	...
	slog.Info("eBPF Statistics Update", // Changed log message slightly
...
func (m *BPFManager) readGlobalStats() (GlobalStats, error) {
	...
	var mapKey uint32 = 0
	// FIX: Use LookupPerCPU here, as it's a PERCPU_ARRAY
	...
	if err != nil {
		if errors.Is(err, ebpf.ErrKeyNotExist) {
			...
			// Return zero stats, but not an error
...
// FIX: Implement statsUpdater method
func (m *BPFManager) statsUpdater(ctx context.Context) {
	...
	if interval <= 1*time.Second {
		interval = 15 * time.Second // Default or fallback
	...
	for {
		...
		case <-ticker.C:
			if err := m.updateAndLogStats(); err != nil {
				// Error is already logged within updateAndLogStats or readGlobalStats
...
func (m *BPFManager) Start(ctx context.Context, wg *sync.WaitGroup) error {
	...
	go func() {
		defer wg.Done()
		// FIX: Call the implemented statsUpdater method
		m.statsUpdater(ctx)
	}()
...
func (m *BPFManager) Close() error {
	...
	m.stopOnce.Do(func() {
		...
		if m.notificationReader != nil {
			...
			// FIX: Check for ringbuf.ErrClosed instead of ebpf.ErrClosed
		...
		for i, l := range links {
			if l != nil {
				...
				if err := l.Close(); err != nil {
					// FIX: Check ebpf.ErrNotAttached correctly
		...
		if m.notificationChannel != nil {
			// Check if already closed before closing
			select {
			case <-m.notificationChannel:
				// Channel is closed or has values (if it has values, closing is problematic, but unlikely here)
			default:
...
// FIX: Implement GetStats method
func (m *BPFManager) GetStats() (GlobalStats, error) {
	m.statsCache.mu.Lock()
	defer m.statsCache.mu.Unlock()
	// Return a copy of the cached stats
	statsCopy := m.statsCache.lastTotalStats
	// Check if the cache has ever been updated (optional, depends on requirements)
	// if m.statsCache.lastStatsTime.IsZero() {
	//     slog.Debug("GetStats called before initial stats update")
	// }
	return statsCopy, nil // No error expected from reading cache
}

func (m *BPFManager) GetNotificationChannel() <-chan NotificationTuple {
	// FIX: Return the channel with the Go NotificationTuple type
...
func (m *BPFManager) UpdateTargetPorts(ports []int) error {
	...
	if err := iter.Err(); err != nil {
		...
		// Don't reset map, try to work with what was read
		// currentPortsMap = make(map[uint16]bool)
	}
