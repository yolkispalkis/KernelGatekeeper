...
struct {
    ...
    __type(key, __u32); // User ID (UID)
    __type(value, struct client_process_info_t);
...
struct {
    ...
    __uint(max_entries, 1 * 1024 * 1024); // 1 MB buffer
...
struct {
    ...
    __uint(max_entries, 2); // 0: total, 1: matched_for_proxy
    __type(key, __u32);
