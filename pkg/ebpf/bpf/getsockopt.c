...
#include <bpf/bpf_core_read.h> // <<< ADD THIS INCLUDE
...
int kernelgatekeeper_getsockopt(struct bpf_sockopt *ctx) {
    ...
    // Read dst_port (peer port in this context)
    // sk_dport is stored in network byte order in struct sock
    __be16 peer_port_n = BPF_CORE_READ(ctx->sk, sk_dport); // Use __be16 and sk_dport field name
    // --- End of BPF_CORE_READ usage ---

    __u16 peer_port_h = bpf_ntohs(peer_port_n); // Convert to host byte order for map key
    ...
    // Cookie lookup remains the same
    ...
    // Original destination lookup remains the same
    ...
    // Buffer validation remains the same
    ...
    // --- Use bpf_probe_write_user remains the same ---
    struct sockaddr_in sa_out = {};
    sa_out.sin_family = AF_INET;
    sa_out.sin_addr.s_addr = orig_dest->dst_ip; // Network byte order
    sa_out.sin_port = orig_dest->dst_port;     // Network byte order
    ...
    // --- End bpf_probe_write_user ---

    // Setting optlen, retval, map cleanup, and logging remain the same
