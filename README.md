...
```yaml
proxy:
  # Тип настройки прокси:
  # "http", "https": Использовать указанный URL прокси статически.
  # "wpad": Использовать WPAD для обнаружения PAC-файла (указать wpad_url).
  # "none": Не использовать прокси (прямое соединение - обычно бесполезно с этой утилитой).
  type: http
  # URL прокси-сервера (если type="http" или "https").
  url: "http://proxy.example.com:3128"
  # URL для загрузки wpad.dat/proxy.pac (если type="wpad").
  # Может быть http(s)://... или file:///path/to/proxy.pac
  wpad_url: "http://wpad.example.com/wpad.dat"
  # Таймаут соединения с прокси (секунды).
  connection_timeout: 10
  # Таймаут выполнения CONNECT-запроса (секунды).
  request_timeout: 30
  # Макс. кол-во попыток подключения к прокси (если используется PAC и он вернул несколько).
  max_retries: 3
  # Кодировка PAC-файла (если не UTF-8). Оставьте пустым для автоопределения.
  pac_charset: ""
  # Таймаут выполнения PAC-скрипта (секунды).
  pac_execution_timeout: 5

kerberos:
  # Realm Kerberos (не обязательно, может быть взят из системной конфигурации).
  realm: "EXAMPLE.COM"
  # Использовать ли кэш учетных данных (ccache)? Обычно true для клиента.
  enable_cache: true
  # Путь к ccache (не обязательно, используется KRB5CCNAME или /tmp/krb5cc_UID).
  # cache_path: "FILE:/home/user/.krb5/ccache"

ebpf:
  # Порты назначения, трафик к которым будет перехватываться.
  target_ports: [80, 443]
  # Разрешать ли клиентам динамически изменять список портов через IPC (пока не реализовано).
...
log_level: "info"
# Путь к лог-файлу службы. Клиент пишет в stderr (управляется systemd --user).
...
socket_path: "/var/run/kernelgatekeeper.sock"
```
